...
object MainScalaObject {
  def main(args: Array[String]): Unit ={
    println("Hello world!")
    val list = List[Int](1,2,3,4,5,6,7,8)
    val list1 = mutable.MutableList[Int](1, 2, 0, 0, -3)
    val list2 = mutable.MutableList[Int]()
    println("Recursive sum: " +sumRecursive(list,list.length))
    println("Tail recursion sum: " + sumTailRecursive(list))
    println( list1 + " <2 previous: " + bigNumNotRec(list1, list2) + "[no_tail]")
    println(list1 + " <2 previous: " + bigNumTail(list1) + "[tail]")
